...
import { InterfaceDeclaration, z } from '@botpress/sdk';
...
export const GenerateContentInputSchema = <S extends z.ZodSchema>(modelRefSchema: S) =>
  z.object({
    ...
    messages: z.array(MessageSchema).min(1).describe('Array of messages for the model to process'), // Enforce at least one message
    responseFormat: z
...
export const llm = new InterfaceDeclaration({
  ...
  actions: {
    generateContent: {
      ...
      cacheable: true,
      input: {
        schema: ({ modelRef }) => GenerateContentInputSchema(modelRef), // Utilize modelRef here
      },
      output: {
